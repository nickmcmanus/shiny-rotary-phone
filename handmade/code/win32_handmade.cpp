...
#define DIRECT_SOUND_CREATE(name) HRESULT WINAPI name(LPCGUID pcGuidDevice, LPDIRECTSOUND *ppDSC, LPUNKNOWN pUnkOuter)
...
Win32InitDSound(HWND Window, int32 SamplesPerSecond, int32 BufferSize)
{
    ...
    if (DSoundLibrary)
    {
        ...
        if (DirectSoundCreate && SUCCEEDED(DirectSoundCreate(0, &DirectSound, 0)))
        {
            ...
            if (SUCCEEDED(DirectSound->SetCooperativeLevel(Window, DSSCL_PRIORITY)))
            {
                ...
                if (SUCCEEDED(DirectSound->CreateSoundBuffer(&BufferDescription, &PrimaryBuffer, 0)))
                {
                    ...
                    if (SUCCEEDED(Error))
                    {
                        ...
                        OutputDebugStringA("Primary Audio Buffer Success!!\n");
                    }
                    else
                    {
                        //TODO: Diagnostic
            ...
            if (SUCCEEDED(Error))
            {
                OutputDebugStringA("Secondary Audio Buffer Success!!\n");
            }
        }
        else
        {
...
#if _WIN32 || _WIN64
#if _WIN64
    OutputDebugStringA("ENVIRONMENT64\n");
#else
    OutputDebugStringA("ENVIRONMENT32\n");
#endif
#endif
            ...
            if(Samples && GameMemory.PermanentStorage && GameMemory.TransientStorage)
            {
                ...
                while (GlobalRunning)
                {
                    ...
                    *NewKeyboardController = ZeroController;
                    ...
                    real64 MSPerFrame = ((1000.0f * (real32)CounterElapsed) / (real64)PerfCountFrequency);
                    real64 FPS = ((real64)PerfCountFrequency / (real64)CounterElapsed);
                    real64 MCPF = ((real64)CyclesElapsed / (1000.0f * 1000.0f));
                    
#if 0
                    char Buffer[256];
                    sprintf_s(Buffer, "%.3fms/f , %.3ff/s , %.3fMc/f\n", MSPerFrame, FPS, MCPF);
